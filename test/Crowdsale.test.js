...
contract('SpinCrowdsale', ([creator, wallet, funder, thirdParty, thirdPartyAlt, address5, address6, address7, address8]) => {
  ...
  const vestAmount_funder = new BigNumber('2.5e21'); // 2,500 SPIN
  const vestAmount_thirdParty = new BigNumber('1e21'); // 1,000 SPIN
  const vestAmount_thirdPartyAlt = new BigNumber('5e21'); // 5,000 SPIN
  ...
  describe('Crowdsale::Admin', () => {
    ...
    it('does not allow an unauthorized address to add a new collector wallet', async () => {
      await this.crowdsale.setWallet(address5, {from: thirdParty}).should.be.rejected;
    });
  ...
  describe('Crowdsale::Phased', () => {
    ...
    it('transfers and locks tokens upon a purchase', async () => {
      ...
      funderPostBalance.should.be.bignumber.equal(funderPreBalance.add(tokenAmount));
  ...
  describe('Crowdsale::Capped', () => {
    ...
    it('does not allow to make a purchase less than the minimum cap', async () => {
      ...
      await this.crowdsale.sendTransaction({from: funder, value: ether(0.1)}).should.be.rejected;
  ...
  describe('Crowdsale::Lockable', () => {
    ...
    it('does not allow to extend lock period and increasing lock amount for a non-existent lock', async () => {
      ...
      await this.crowdsale.adjustLockPeriod(funder, lockReason, lockExpiryDate).should.be.rejected;
  ...
  describe('Crowdsale::Vesting&Releasing', () => {
    ...
    it('releases all locked tokens', async () => {
      ...
      funderPostBalance.should.be.bignumber.equal(funderPreBalance.add(tokenAmount));
      thirdPartyPostBalance.should.be.bignumber.equal(thirdPartyPreBalance.add(tokenAmount));
      thirdPartyAltPostBalance.should.be.bignumber.equal(thirdPartyAltPreBalance.add(tokenAmount));
      ...
      bonusAmount.mul(3).should.be.bignumber.equal(totalLockedTokenFunder
      ...
      funderPostBalance.should.be.bignumber.equal(funderPreBalance.add(tokenAmount).add(bonusAmount));
      thirdPartyPostBalance.should.be.bignumber.equal(thirdPartyPreBalance.add(tokenAmount).add(bonusAmount));
      thirdPartyAltPostBalance.should.be.bignumber.equal(thirdPartyAltPreBalance.add(tokenAmount).add(bonusAmount));
    });
    ...
    it('delivers tokens manually for the purchases through thirdparty ICO platforms', async () => {
      ...
      let funderPostBalance = await this.token.balanceOf(funder);
      let thirdPartyPostBalance = await this.token.balanceOf(thirdParty);
      funderPostBalance.should.be.bignumber.equal(funderPreBalance.add(vestAmount_funder));
      thirdPartyPostBalance.should.be.bignumber.equal(thirdPartyPreBalance.add(vestAmount_thirdParty));
      ...
      funderPostBalance = await this.token.balanceOf(funder);
      thirdPartyPostBalance = await this.token.balanceOf(thirdParty);
      funderPostBalance.should.be.bignumber.equal(funderPreBalance.add(funderBonus));
      thirdPartyPostBalance.should.be.bignumber.equal(thirdPartyPreBalance.add(thirdPartyBonus));
    });
