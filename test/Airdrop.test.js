...
const Airdrop = artifacts.require('SpinAirdrop');
...
contract('Airdrop', ([creator, addr1, addr2, addr3, addr4, addr5]) => {
  const airdropFund = new BigNumber('10000e18');  // 10000 SPIN tokens
  const withdrawalAmount = new BigNumber('100e18');  // 100 SPIN tokens
  ...
  describe('Airdrop::functions', () => {
    it('sets token to be airdropped', async () => {
      let newToken = await SpinToken.new(NAME, SYMBOL, DECIMALS, INITIAL_SUPPLY);
    ...
    it('withdraws token from the contract', async () => {
      ...
      await this.token.transfer(this.airdrop.address, airdropFund).should.be.fulfilled;
  ...
  describe('Airdrop::authority', () => {
    it('does not allows an unauthorized address to set token to be airdropped', async () => {
      let newToken = await SpinToken.new(NAME, SYMBOL, DECIMALS, INITIAL_SUPPLY);
      await this.airdrop.setToken(newToken.address, {from: addr1}).should.be.rejected;
    });

    it('does not allow an unauthorized address to withdraw token from the contract', async () => {
      await this.token.transfer(this.airdrop.address, airdropFund).should.be.fulfilled;
      // Withdraw some tokens
      await this.airdrop.withdrawToken(withdrawalAmount, {from: addr1}).should.be.rejected;
    });

    it('does not allow an unauthorized address to withdraw ether from the contract', async () => {
      await this.airdrop.withdrawToken(ether(1), {from: addr1}).should.be.rejected;
    });
