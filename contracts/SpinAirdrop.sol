...
contract SpinAirdrop is Withdrawable, AdminRole {
   ...
   */
  function bulkAirdrop(address[] airdropees, uint256[] amounts) external onlyAdmin {
    for (uint256 i = 0; i < airdropees.length; i++) {
      require(_token.transfer(airdropees[i], amounts[i]), "Transfer failed");
    }
  }
